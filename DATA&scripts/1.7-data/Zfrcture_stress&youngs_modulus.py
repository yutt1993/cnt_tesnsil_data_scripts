# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 11:08:36 2024
...
for directory in directories:
    ...
    fracture_data.append({
        ...
        'Material': 'CNT(12)' if directory == directories[0] else '(C$_{60})$@SWCNT_Zigzag_chain(12)'
    })

    ax1.plot(angles_sorted, fracture_stress_values_sorted, 'o-', label='CNT(12)' if directory == directories[0] else '(C$_{60})$@SWCNT_Zigzag_chain(12)')
...
ax2.plot(all_dir_names[0], all_slopes[0], 'o-', label='CNT(12)')
ax2.plot(all_dir_names[1], all_slopes[1], 'o-', label='(C$_{60})$@SWCNT_Zigzag_chain(12)')
...
fracture_df_columns_1.to_csv(r'../1.7_data/Zfracture_stress_data_part1.csv', index=False)
fracture_df_columns_2.to_csv(r'../1.7_data/Zfracture_stress_data_part2.csv', index=False)
...
youngs_modulus_df_columns_1.to_csv(r'../1.7_data/Zyoungs_modulus_data_part1.csv', index=False)
youngs_modulus_df_columns_2.to_csv(r'../1.7_data/Zyoungs_modulus_data_part2.csv', index=False)

# 保存并展示图表
plt.savefig(os.path.join(r'../1.7_data/Z.pdf'))
plt.show()